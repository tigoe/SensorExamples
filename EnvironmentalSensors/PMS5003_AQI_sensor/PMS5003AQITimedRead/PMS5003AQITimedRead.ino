/*
  ...
  This version reads constantly and does a timed printing once a minute.
...
long lastRead = 0;
// last time the sensor was reset:
long lastReset = 0;
// state of the reset pin:
bool resetState = LOW;
bool lastResetState = HIGH;

// interval between reads, in ms:
const long readInterval = 60 * 1000;
// reset time in ms:
const long resetInterval = 30 * 1000;
const int sensorResetPin = 2;
...
void loop() {
  // declare a buffer for the raw bytes from the sensor:
  byte dataBuffer[32];
  ...
  if (processData(dataBuffer, result)) {
    // if you're storing the data somewhere, or sending it to a database,
    // this is where you would do it.
    // Printing the values, shown below, is just a stand-in for those actions:

    // print the readings once a minute:
    if (millis() - lastRead > readInterval) {
      ...
      // notify that you had a successful read:
